...
def single_thread_download(url, dst):
    ...
    print(requests.head("https://bmclapi2.bangbang93.com" + requests.head(url).headers['Location']).headers)
    file_size = int(requests.head("https://bmclapi2.bangbang93.com" + requests.head(url).headers['Location']).headers[
                        'Content-Length'])
    ...
    pbar = tqdm(
        ...
        unit='B', unit_scale=True, desc=url.split('/')[-1], ncols=100)
...
if __name__ == '__main__':
    ...
    single_thread_download('https://bmclapi2.bangbang93.com/version/1.7.10/server', './1.jar')
    end = time.perf_counter()
    print('[Message] Running time: %s Seconds' % (end - start))

    many_thread_download = ManyThreadDownload()
    start = time.perf_counter()
    many_thread_download.run('https://bmclapi2.bangbang93.com/version/1.7.10/server', './2.jar')
    end = time.perf_counter()
    print('\n')
    print('[Message] Running time: %s Seconds' % (end - start))
